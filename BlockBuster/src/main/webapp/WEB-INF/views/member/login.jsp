...
function checkIdPw(){
	...
	$.ajax({	
		...
		success:function(data){
			console.log(data);
			if(data == '0'){
				$("<p id='message'>아이디와 비밀번호를 다시 확인해주세요</p>").insertBefore("#pwbelow");
				return false;
			}else if(data == '1'){
				var form = document.login;
				 form.submit();
			}else if(data == '2'){
				$("<p id='message'>탈퇴한 회원입니다.관리자에게 문의하세요</p>").insertBefore("#pwbelow");
				return false;
			}
		}
	});
...
<section class="login-form">
	...
	<form method="get" name="login" action="${pageContext.request.contextPath}/login" onsubmit="checkIdPw(); return false;">
		...
		<div class="int-area">	
			...
			<label for="id" id="pwbelow">PASSWORD</label>
		</div>
		<div class="btn-area">
